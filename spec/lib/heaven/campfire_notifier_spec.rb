...
describe "Heaven::Notifier::Campfire" do
  it "handles pending notifications" do
    n = Heaven::Notifier::Campfire.new(JSON.parse(fixture_data("deployment-pending")))
  ...
  it "handles successful deployment statuses" do
    n = Heaven::Notifier::Campfire.new(JSON.parse(fixture_data("deployment-success")))
    expect(n.default_message).to \
  ...
  it "handles failure deployment statuses" do
    n = Heaven::Notifier::Campfire.new(JSON.parse(fixture_data("deployment-failure")))
    expect(n.default_message).to \
